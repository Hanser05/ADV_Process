...
%%
%TKE:[burst,u'2,v'2,w'2,UL'2]
%UL'2：水平向主流向，湍流动能
%tc:[burst,tc1-1(KeepNegtiveValue),tc1-2(DeleteNegtiveValue),tc2-1(),tc2-2()]
%tc1:u',v'，w'全部去波轨流速，“-1”不去负值，“-2”去掉负值
%tc2:u',v'去波轨流速，w'不去波轨流速，“-1”不去负值，“-2”去掉负值
%%
Year=ADV5049_vhd(:,3);
Mon=ADV5049_vhd(:,1);
Day=ADV5049_vhd(:,2);
...
%       erate(i)=(1/a*Sww_f_mean2).^(3/2)*2*pi/abs(Ubar(i));
        Fl = uvw/dis; % 惯性耗散副区的lower limit 参考自Stapleton and Huntley, 1995
        Fu = fs/2;  % 惯性耗散副区的upper limit, 经过与-5/3率对比，发现Fs/2的范围内都符合      
        
        % turb1d
        [f1,s1]=turb1d_ef(turW1_dwave,uvw1,fs,4,N/4,2);
        F1=f1(f1<Fu&f1>Fl);S1=s1(f1<Fu&f1>Fl);
        tmp1=F1.^(5/3).*S1;
        Ustar_IDw1(i)=(2*pi*0.4*dis1/uvw1)^(1/3)*(mean(tmp1)/0.69)^0.5;
        Touc_ID_smooth(i)=density*Ustar_IDw1(i).^2;
        ...
        tmp2=F2.^(5/3).*S2;
        Ustar_IDw2(i)=(2*pi*0.4*dis1/uvw1)^(1/3)*(mean(tmp2)/0.69)^0.5;
        Touc_ID_pw(i)=density*Ustar_IDw2(i).^2;
