function [Su,Sp,Dir,Spread,F,dF,DOF,AA] = wds2014(u,v,p,dt,nF,waterdepth,hp,hv,parms);
...
mp=ones(lF,1)*(waterdepth-hp);        % vertical scaling
...
BB=(sinhkh./(2*pi*F2)./coshkhZ).^2;
% BB(find(F2>hf))=1;
Su=(Cuu+Cvv).*BB;
AA=(coshkh./coshkhz).^2;
AA(find(F2>hf))=1;  % qubin: make higher freq scaling = 1 
Sp=Cpp.*AA;
